...
def add_recipe():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if request.args.get('recipe'):
            recipe_id = request.args.get('recipe')
            copy_recipe = "INSERT INTO recipes (recipe_id, user_id, title, description) SELECT UUID(), (SELECT id FROM verifiedusers WHERE username='dinglesnoot'), CASE WHEN (SELECT title FROM recipes WHERE recipe_id=%s) IN (SELECT title FROM recipes) THEN CONCAT((SELECT title FROM recipes WHERE recipe_id=%s),'-',UUID()) ELSE (SELECT title FROM recipes WHERE recipe_id=%s) END, description FROM recipes WHERE recipe_id=%s;"
